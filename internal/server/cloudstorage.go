...
func (s *Server) getCloudFileslistByUser(ctx context.Context, userID string) ([]*FileInfo, error) {
	...
	for _, fileInfo := range filesInfo {
		...
		sanitizedFilesInfo = append(sanitizedFilesInfo, &FileInfo {
			...
			LastEditedAt: fileInfo.LastEditedAt,
		})
	}
...
// func (s *Server) addStorageFile(ctx context.Context, userID, propertyID, fileName string) error {
...
// 	wc := s.StorageClient.Bucket(s.UsersBucket).Object(key).NewWriter(tCtx)
